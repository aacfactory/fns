	...
	if function.Param != nil {
		...
		if hasCache && function.Result != nil {
			if cacheCmd == "get" || cacheCmd == "get-set" {
				...
				body.Tab().Tab().Tab().Token("err = errors.Wrap(decodeErr)").Line()
				...
				body.Tab().Tab().Token("if log.WarnEnabled() {").Line()
				body.Tab().Tab().Tab().Token("log.Warn().Cause(err).With(\"fns\", \"caches\").Message(\"fns: scan cached value failed\")").Line()
				body.Tab().Tab().Token("}").Line()
				body.Tab().Token("}").Line()
			}
		}
