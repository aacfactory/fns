...
func (s *ServiceFile) functionProxyCode(ctx context.Context, function *sources.Function) (code gcg.Code, err error) {
	...
	if function.Param != nil {
		if function.Result == nil {
			body.Tab().Token(fmt.Sprintf("_, handleErr := eps.Request(ctx, _endpointName, %s, param)", function.ConstIdent)).Line()
		} else {
			body.Tab().Token(fmt.Sprintf("response, handleErr := eps.Request(ctx, _endpointName, %s, param)", function.ConstIdent)).Line()
		}
	} else {
		if function.Result == nil {
			body.Tab().Token(fmt.Sprintf("_, handleErr := eps.Request(ctx, _endpointName, %s, nil)", function.ConstIdent)).Line()
		} else {
			body.Tab().Token(fmt.Sprintf("response, handleErr := eps.Request(ctx, _endpointName, %s, nil)", function.ConstIdent)).Line()
		}
	}
