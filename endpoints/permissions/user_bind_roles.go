...
func UserBindRoles(ctx context.Context, userId string, roles ...string) (err errors.CodeError) {
	...
	if !hasRequest {
		err = errors.Warning("permissions: user bind roles failed").WithCause(fmt.Errorf("there is no request in context"))
		return
	}
	...
	fr := endpoint.Request(ctx, "user_bind_roles", service.NewArgument(&permissions.UserBindRolesArgument{
		UserId: userId,
		Roles:  roles,
	}))
