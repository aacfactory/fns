...
func Bind(ctx context.Context, subject string, roles ...string) (err errors.CodeError) {
	...
	if subject == "" {
		err = errors.ServiceError("rbac endpoint bind role failed").WithCause(fmt.Errorf("subject is nil"))
		return
	}
	if roles == nil || len(roles) == 0 {
		err = errors.ServiceError("rbac endpoint bind role failed").WithCause(fmt.Errorf("roles is nil"))
		return
	}
