...
func ConvertHttpHandlerFunc(h http.HandlerFunc) Handler {
	return ConvertHttpHandler(h)
}

func ConvertHttpHandler(h http.Handler) Handler {
	return HandlerFunc(func(writer ResponseWriter, request Request) {
		...
		for k, vv := range w.Header() {
			if k == bytex.ToString(ContentTypeHeaderName) {
		...
		if !haveContentType {
			...
			writer.Header().Set(ContentTypeHeaderName, bytex.FromString(http.DetectContentType(b[:l])))
		}
	})
}

func ConvertRequest(ctx Request, r *http.Request, forServer bool) error {
